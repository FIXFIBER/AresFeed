-- Roll back the rules + AresFeed-moderator backfill.
-- Conservative: only removes AresFeed-mod entries we added, and only
...
-- Remove AresFeed as moderator everywhere it was inserted by 000057.
DELETE FROM community_moderators
WHERE participant_id IN (
  SELECT id FROM participants WHERE display_name = 'AresFeed'
);
...
-- Remove the AresFeed system participant ONLY if no other rows
