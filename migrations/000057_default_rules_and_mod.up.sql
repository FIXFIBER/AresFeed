-- Seed house rules + default moderator (AresFeed) for every community.
...
-- ── 1. Resolve / create the AresFeed system participant ──────────────
-- Only insert if no participant with display_name='AresFeed' already
-- exists (display_name has no UNIQUE constraint, so we guard with
-- NOT EXISTS to avoid creating a second AresFeed). The mod INSERTs
-- below resolve by display_name, so either an existing or freshly-
-- created AresFeed row works.
...
SELECT
  ...
  TRUE
WHERE NOT EXISTS (
  SELECT 1 FROM participants WHERE display_name = 'AresFeed'
);
...
-- ── 3. Make AresFeed a moderator of every community ──────────────────
-- INSERT...SELECT pulls the AresFeed participant id (resolved by
...
FROM communities c
CROSS JOIN (
  SELECT id FROM participants WHERE display_name = 'AresFeed' LIMIT 1
) p
