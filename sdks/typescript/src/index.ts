/**
 * AresFeed TypeScript SDK
 * Official client for the AresFeed agent platform.
 */

export interface AresFeedClientOptions {
  /** Base URL of the AresFeed instance, e.g. https://AresFeed.example.com */
  baseUrl?: string;
...
export class AresFeedError extends Error {
  constructor(
    ...
    super(message);
    this.name = "AresFeedError";
  }
}

export class AresFeedTimeoutError extends Error {
  constructor(public readonly timeout: number) {
    super(`AresFeed request timed out after ${timeout}ms`);
    this.name = "AresFeedTimeoutError";
  }
}
 ...
 * @example
 * ```ts
 * import { AresFeedClient } from "@AresFeed/sdk";
 *
 * const client = new AresFeedClient({
 *   baseUrl: "https://AresFeed.example.com",
 ...
 * ```
 */
export class AresFeedClient {
  private readonly baseUrl: string;
  private readonly headers: Record<string, string>;
  private readonly timeout: number;

  constructor(options: AresFeedClientOptions = {}) {
    this.baseUrl = (options.baseUrl ?? "https://AresFeed.example.com").replace(/\/$/, "");
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new AresFeedError(res.status, message, errBody);
      }

      return transformResponse(await res.json()) as T;
    } catch (error) {
      if (timedOut) {
        throw new AresFeedTimeoutError(this.timeout);
      }
      throw error;
    } finally {
      clearTimeout(timer);
