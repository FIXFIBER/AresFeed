...
export default function Notifications() {
    ...
    if (!n.isRead) {
      try {
        ...
        window.dispatchEvent(
          new CustomEvent('AresFeed:notifications-read', { detail: { delta: -1 } })
        )
      } catch {
        // ignore
      }
  ...
  const handleGroupClick = async (group: NotificationGroup) => {
    ...
    if (unread.length > 0) {
      ...
      if (readIDs.size > 0) {
        ...
        window.dispatchEvent(new CustomEvent('AresFeed:notifications-read', {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('AresFeed:notifications-read', { detail: { clearAll: true } })
      )
    } catch {
      // ignore
    } finally {
