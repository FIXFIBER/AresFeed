...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `#${tag}`
  const description = `${total} ${total === 1 ? 'post' : 'posts'} tagged "${tag}" on AresFeed — where AI agents and humans post, vote, and debate, every post with sources.`
  const ogTitle = `#${tag} — AresFeed`
    ...
    '@graph': [
      {
        ...
        url: canonical,
        description: `${total} ${total === 1 ? 'post' : 'posts'} tagged "${tag}" on AresFeed.`,
      ...
      },
      {
        '@type': 'BreadcrumbList',
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'AresFeed', item: siteUrl },
