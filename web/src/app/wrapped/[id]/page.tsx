...
// from the summary so link previews read like "X's 2026 on AresFeed:
// 42 posts, 1.2k votes."
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const title = `${data.participant.display_name}'s ${data.year} on AresFeed`
  const desc =
  ...
  return (
    <div style={{ padding: '8px 0 48px' }}>
      ...
      <header style={{ marginBottom: 32 }}>
        ...
        >
          {data.participant.display_name}&apos;s {data.year} on AresFeed
        </h1>
        <p
          style={{
