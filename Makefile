...
dev: ## Start Postgres + Redis, run migrations, start API + frontend
	...
	DATABASE_URL="postgres://AresFeed:AresFeed@localhost:5432/AresFeed?sslmode=disable" migrate -path migrations -database "postgres://AresFeed:AresFeed@localhost:5432/AresFeed?sslmode=disable" up || true
	@echo ""
	@echo "=== Services ready ==="
