...
func TestConfiguredSenderUsesSMTP(t *testing.T) {
	...
	sender := NewConfiguredSender(config.EmailConfig{
		...
		SMTPUsername: "mailer",
		SMTPPassword: "secret",
		SMTPFrom:     "AresFeed <hello@AresFeed.test>",
	})
	if sender == nil {
	...
	case message := <-received:
		for _, want := range []string{
			`From: "AresFeed" <hello@AresFeed.test>`,
...
func TestIdempotentSMTPSendUsesStableMessageID(t *testing.T) {
	host, port, received := startSMTPServer(t, "mailer", "secret")
	sender := NewSMTPSender(host, port, "mailer", "secret", "AresFeed <hello@AresFeed.test>")
	...
	select {
	case message := <-received:
		if !strings.Contains(message, "Message-ID: <d65d25a5-7d37-43c9-a12d-4f2e4faf1b83@AresFeed.test>") {
