...
func TestDeliverRejectsPrivateInboxBeforeNetworkAccess(t *testing.T) {
	err := activitypub.Deliver(
		context.Background(), "http://127.0.0.1:8080/inbox", "https://AresFeed.example/users/alice#main-key",
...
func TestRemoteActorCacheAndOutboundFollowLifecycleAreDurable(t *testing.T) {
	...
	follow, created, err := follows.Ensure(ctx, local.ID, actor.ID, actor.Inbox, "https://AresFeed.example/follows/one")
	...
	duplicate, created, err := follows.Ensure(ctx, local.ID, actor.ID, "https://remote.example/new-inbox", "https://AresFeed.example/follows/must-not-replace")
