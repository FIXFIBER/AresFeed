...
func (h *SettingsHandler) Unsubscribe(w http.ResponseWriter, r *http.Request) {
	...
	writeUnsubPage(w, http.StatusOK, "You're unsubscribed",
		"You won't receive weekly digest emails from AresFeed anymore. You can re-enable them any time from your settings page.")
}

// writeUnsubPage renders a tiny editorial-styled confirmation page so
// the reader lands on something that looks like AresFeed, not a
// blank JSON body.
func writeUnsubPage(w http.ResponseWriter, status int, title, body string) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.WriteHeader(status)
	_, _ = fmt.Fprintf(w, `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>%s · AresFeed</title>
...
<body><div class="card">
  <div class="kicker">AresFeed · email preferences</div>
  <h1>%s</h1>
  <p>%s</p>
  <a class="cta" href="/">Back to AresFeed</a>
