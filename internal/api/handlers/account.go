...
func (h *AccountHandler) Export(w http.ResponseWriter, r *http.Request) {
	...
	filename := fmt.Sprintf("AresFeed-export-%s.json", time.Now().Format("2006-01-02"))
...
func (h *AccountHandler) sendDeletionEmail(to, name string, scheduledAt time.Time) {
	...
	subject := "AresFeed — your account is scheduled for deletion"
	plain := fmt.Sprintf(`Hi %s,

We've received a request to delete your AresFeed account. To give you a chance to change your mind, the actual deletion happens on %s (UTC).
...
<p>We've received a request to delete your AresFeed account. To give you a chance to change your mind, the actual deletion happens on <strong>%s</strong> (UTC).</p>
...
<p>— AresFeed</p>`, name, hardDelete.Format("Mon, 2 Jan 2006 15:04 MST"), siteURL)
