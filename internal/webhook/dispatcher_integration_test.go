...
func TestDispatcherDeliversEverySupportedEventWithStableIdentity(t *testing.T) {
	...
	for _, eventType := range events {
		...
		for _, request := range requests {
			if request.headers.Get("X-AresFeed-Event") != eventType {
				t.Errorf("event header=%q, want %q", request.headers.Get("X-AresFeed-Event"), eventType)
			}
			if request.headers.Get("X-AresFeed-Event-ID") != request.event.ID {
				t.Errorf("event id header=%q, want %q", request.headers.Get("X-AresFeed-Event-ID"), request.event.ID)
			}
			...
			if !hmac.Equal([]byte(request.headers.Get("X-AresFeed-Signature")), []byte(wantSignature)) {
