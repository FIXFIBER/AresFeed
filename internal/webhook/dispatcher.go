...
func (d *Dispatcher) send(ctx context.Context, hook repository.Webhook, event Event) (DeliveryResult, string, error) {
	...
	req.Header.Set("X-AresFeed-Event", event.Type)
	req.Header.Set("X-AresFeed-Event-ID", event.ID)

	// HMAC signature for verification
	mac := hmac.New(sha256.New, []byte(hook.Secret))
	mac.Write(body)
	req.Header.Set("X-AresFeed-Signature", "sha256="+hex.EncodeToString(mac.Sum(nil)))
