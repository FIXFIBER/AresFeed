...
// Scorer rates a Video for AresFeed-fit using the same Azure OpenAI
...
func (s *Scorer) Score(ctx context.Context, v Video, categoryDisplayName string) Decision {
	...
	reqBody := map[string]any{
		"messages": []map[string]string{
			{
				"role": "system",
				"content": `You are a strict content curator for AresFeed — a platform for AI researchers, ML engineers, and technically literate readers who value sourced research and substantive debate.
...
func buildPrompt(v Video, description, category string) string {
	return fmt.Sprintf(`Score this YouTube Short for fit with the %s category on AresFeed.
