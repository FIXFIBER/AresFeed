...
import (
	...
	"github.com/FIXFIBER/AresFeed/internal/email"
...
func RunPeriod(ctx context.Context, cfg Config, period Period) (int, error) {
	...
	lockKey := fmt.Sprintf("AresFeed:digest:%s:%s", period.Cadence, period.Start.Format(time.RFC3339Nano))
	var acquired bool
...
func digestSubject(postCount int, cadence Cadence) string {
	...
	return fmt.Sprintf("Top %d AresFeed posts %s", postCount, window)
}
...
func renderDigest(
	...
	htmlSb.WriteString(fmt.Sprintf(`<p style="color:#52525b;font-size:15px;margin:0 0 24px">Here are the top %d posts on AresFeed %s.</p>`, len(posts), window))

	plainSb.WriteString(fmt.Sprintf("Hi %s,\n\n", r.DisplayName))
	plainSb.WriteString(fmt.Sprintf("Here are the top %d posts on AresFeed %s:\n\n", len(posts), window))
	...
	htmlSb.WriteString(fmt.Sprintf(`<div style="text-align:center;margin:32px 0 8px"><a href="%s" style="display:inline-block;background:#18181b;color:#fff;text-decoration:none;padding:12px 24px;border-radius:10px;font-weight:600;font-size:14px">Open AresFeed</a></div>`, siteURL))
	htmlSb.WriteString(fmt.Sprintf(`<p style="color:#a1a1aa;font-size:12px;text-align:center;margin-top:24px">You received this because you verified your email on AresFeed.<br/><a href="%s/settings" style="color:#a1a1aa">Manage email preferences</a>  ·  <a href="%s" style="color:#a1a1aa">Unsubscribe</a></p>`, siteURL, unsubURL))
	htmlSb.WriteString(`</body></html>`)

	plainSb.WriteString(fmt.Sprintf("Open AresFeed: %s\n\nYou received this because you verified your email.\nManage preferences: %s/settings\nUnsubscribe (one click): %s\n", siteURL, siteURL, unsubURL))
