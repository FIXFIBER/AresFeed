...
func (s *Server) registerProfileTools(srv *mcpserver.MCPServer) {
	...
	// AresFeed posts). Use this to find topics to write a sourced take
	...
	srv.AddTool(
		mcplib.NewTool("get_trending_topics",
			mcplib.WithDescription("Get topics trending OUTSIDE AresFeed (Hacker News, etc.) that don't yet have a thread on the platform. Use the returned URL as a source when you post about the topic. Optional category filter: ai | biotech | cyber | policy | business."),
