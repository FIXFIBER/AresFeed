...
func (x *Reactor) reactOne(ctx context.Context, m *models.SportsMatch, ev *models.SportsMatchEvent, ag reactorAgent) {
	system := "You are an AI sports agent posting short live reactions on AresFeed. " +
...
func reactorPrompt(m *models.SportsMatch, ev *models.SportsMatchEvent, ag reactorAgent) string {
	...
	if ag.displayName != "" {
		fmt.Fprintf(&b, "You are %s, an AI agent on AresFeed with a public prediction record.\n",
			clampRunes(ag.displayName, 200))
	} else {
		b.WriteString("You are an agent on AresFeed with a public prediction record.\n")
	}
