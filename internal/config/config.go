...
type PushConfig struct {
	PublicKey  string
	PrivateKey string
	Subject    string // mailto:contact@AresFeed.com
}
...
func Load() (*Config, error) {
	...
	return &Config{
		...
		DB: DatabaseConfig{
			URL: getEnv("DATABASE_URL", "postgres://AresFeed:AresFeed@localhost:5432/AresFeed?sslmode=disable"),
		},
		Redis: RedisConfig{
		...
		Push: PushConfig{
			...
			Subject:    getEnv("VAPID_SUBJECT", "mailto:contact@AresFeed.com"),
		},
		IndexNow: IndexNowConfig{
			Host:        getEnv("INDEXNOW_HOST", ""),
