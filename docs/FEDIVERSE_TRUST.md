...
AresFeed computes a `trust_score` per participant based on local
...
- A remote Mastodon user who sees a post from `@alice@AresFeed.com`
  cannot verify *why* Alice has a trust score of 87.
- Conversely, when `@bob@mastodon.social` follows and replies on
  AresFeed, **we** have no trust context for Bob — his Mastodon
...
- **Ignore cross-instance trust entirely** → silos AresFeed's best
...
1. **Signals are local, trust is local.** We never trust another
   instance's self-reported score as authoritative. A AresFeed trust
   score only carries weight *on AresFeed*, and a remote trust score
...
2. **Attestations are a hint, not proof.** We emit signed
   ...
   *choose* to weight our number. They carry cryptographic integrity
   (the signature proves AresFeed issued the number) but not
   normative weight (each receiver decides what to do with it).

3. **Earn trust here via observable behavior.** Remote actors start
   at a low base trust score on AresFeed. They earn it the same way
...
Extend the ActivityStreams `@context` on every AresFeed actor with a
custom vocabulary:
...
```json
{
  "id": "https://www.AresFeed.com/users/alice",
  "type": "Person",
  ...
  "trust": {
    "score": 87.3,
    "scale": "0-100",
    "issuer": "https://www.AresFeed.com",
...
When a remote actor interacts with AresFeed (follows, replies,
...
- ✅ Whether AresFeed users chose to follow them back
...
lets federated comments and votes reuse AresFeed's normal author, thread, and
...
- **Reputation portability.** A user leaving Mastodon for AresFeed
