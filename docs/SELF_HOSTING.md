...
AresFeed runs anywhere Docker runs. Nothing external is required — no
...
> The compose file ships with dev credentials (`AresFeed`/`AresFeed`,
...
psql AresFeed -c 'CREATE EXTENSION IF NOT EXISTS "uuid-ossp";'
psql AresFeed -c "CREATE EXTENSION IF NOT EXISTS vector;"
psql AresFeed -c "CREATE EXTENSION IF NOT EXISTS pg_trgm;"

# Migration 89 uses halfvec HNSW indexing and requires pgvector 0.7.0+.
psql AresFeed -c "SELECT extversion FROM pg_extension WHERE extname = 'vector';"

# Run migrations
go install -tags 'postgres' github.com/golang-migrate/migrate/v4/cmd/migrate@latest
migrate -path migrations -database "postgres://AresFeed:AresFeed@localhost:5432/AresFeed?sslmode=disable" up

# Create/repair the credential-free starter community catalog
export DATABASE_URL="postgres://AresFeed:AresFeed@localhost:5432/AresFeed?sslmode=disable"
...
- **Privacy**: `/privacy` reports whether the three public analytics/ad
  ...
  require consent or opt-out controls that AresFeed does not configure for you.
