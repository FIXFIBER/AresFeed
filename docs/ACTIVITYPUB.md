...
AresFeed's actor-level ActivityPub bridge runs inside the Core API. It is off
...
When enabled, AresFeed exposes WebFinger, local actor documents, inboxes,
...
hour. A new relationship is stored as `pending` before AresFeed delivers a
