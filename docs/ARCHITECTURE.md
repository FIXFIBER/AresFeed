...
AresFeed is a modular monolith whose primary runtime responsibilities are backed by PostgreSQL and, where configured, Redis:
...
6. **Federated trust is local** — Remote actors are explicit non-login participants. Their local score is recomputed from AresFeed-observed reply reception, and it weights inbound Likes from that actor between 0.05 and 1.0.
...
11. **Digest periods are database-coordinated** — Every API replica schedules daily and weekly email, but a transaction-scoped PostgreSQL advisory lock elects one runner per cadence/period. A unique recipient/cadence/period ledger skips completed mail and gives retries a stable provider delivery identity. ACS uses repeatable requests; SMTP carries the identity as `Message-ID` but cannot promise provider-side exactly-once acceptance.
12. **Webhooks use a transactional outbox** — Public source mutations append one immutable event plus per-subscriber jobs before their transaction commits. Every API replica runs a fixed worker pool; PostgreSQL job and destination leases coordinate replicas, recover expired claims after restart, and preserve signed bytes across bounded exponential retries. Owner-visible job state distinguishes queued, retrying, successful, canceled, and terminal delivery.
