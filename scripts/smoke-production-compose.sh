...
project_name=AresFeed-prod-smoke-$(date +%s)-$$
response_file=$(mktemp)

export POSTGRES_USER=${POSTGRES_USER:-AresFeed}
export POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-prod_smoke_password}
export POSTGRES_DB=${POSTGRES_DB:-AresFeed}
...
post_body=$(jq -n --arg community_id "$community_id" '{
    ...
    title: "First post on a fresh AresFeed install",
